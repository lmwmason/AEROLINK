...
// Software-only AEROLINK transport. Runtime activation still requires the
// explicit --aerolink-vehicle argument; it is never enabled by EEPROM default.
#ifdef USE_AEROLINK
#define AEROLINK_UART SERIAL_PORT_USART8
#endif
...
unsigned targetGetSitlInstance(void);
unsigned targetGetAerolinkVehicleId(void);
