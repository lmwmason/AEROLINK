...
#define PORT_PWM_RAW_BASE 9001
#define PORT_PWM_BASE     9002
#define PORT_STATE_BASE   9003
#define PORT_RC_BASE      9004
#define INSTANCE_PORT_STRIDE 16

static unsigned sitlInstance;
static unsigned aerolinkVehicleId;

unsigned targetGetSitlInstance(void) { return sitlInstance; }
unsigned targetGetAerolinkVehicleId(void) { return aerolinkVehicleId; }
...
int targetParseArgs(int argc, char * argv[])
{
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            ...
            printf("  --instance <0..63> Offset TCP/UDP ports for concurrent SITL\n");
            printf("  --aerolink-vehicle <1..15> Enable non-actuating UART8 endpoint\n");
            ...
            if (!end || *end || value > 63) { fprintf(stderr, "[SITL] invalid instance\n"); exit(1); }
            sitlInstance = (unsigned)value;
        } else if (strcmp(argv[i], "--aerolink-vehicle") == 0 && i + 1 < argc) {
            char *end = NULL; unsigned long value = strtoul(argv[++i], &end, 10);
            if (!end || *end || value < 1 || value > 15) { fprintf(stderr, "[SITL] invalid AEROLINK vehicle\n"); exit(1); }
            aerolinkVehicleId = (unsigned)value;
        } else {
    ...
    const unsigned instanceOffset = sitlInstance * INSTANCE_PORT_STRIDE;
    printf("[SITL] instance=%u output %s:%u (Gazebo) and %s:%u (RealFlightBridge)\n",
           sitlInstance, simulator_ip, PORT_PWM_BASE + instanceOffset,
           simulator_ip, PORT_PWM_RAW_BASE + instanceOffset);
    return 0;
}
...
void systemInit(void)
{
    ...
    const int offset = (int)sitlInstance * INSTANCE_PORT_STRIDE;
    ret = udpInit(&pwmLink, simulator_ip, PORT_PWM_BASE + offset, false);
    printf("[SITL] init PwmOut UDP link to gazebo %s:%d...%d\n", simulator_ip, PORT_PWM_BASE + offset, ret);

    ret = udpInit(&pwmRawLink, simulator_ip, PORT_PWM_RAW_BASE + offset, false);
    printf("[SITL] init PwmOut UDP link to RF9 %s:%d...%d\n", simulator_ip, PORT_PWM_RAW_BASE + offset, ret);

    ret = udpInit(&stateLink, NULL, PORT_STATE_BASE + offset, true);
    printf("[SITL] start UDP server @%d...%d\n", PORT_STATE_BASE + offset, ret);

    ret = udpInit(&rcLink, NULL, PORT_RC_BASE + offset, true);
    printf("[SITL] start UDP server for RC input @%d...%d\n", PORT_RC_BASE + offset, ret);
