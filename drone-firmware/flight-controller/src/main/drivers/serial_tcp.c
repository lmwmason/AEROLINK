...
#define INSTANCE_PORT_STRIDE 64

extern unsigned targetGetSitlInstance(void);
...
static tcpPort_t* tcpReconfigure(tcpPort_t *s, int id)
{
    ...
    const unsigned listenPort = BASE_PORT + targetGetSitlInstance() * INSTANCE_PORT_STRIDE + id + 1;
    if (dyad_listenEx(s->serv, NULL, listenPort, 10) == 0) {
        fprintf(stderr, "bind port %u for UART%u\n", listenPort, (unsigned)id + 1);
    } else {
